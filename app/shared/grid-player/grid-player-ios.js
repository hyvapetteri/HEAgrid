...
var GridPlayer = /** @class */ (function (_super) {
    ...
    GridPlayer.prototype.initialize = function (options) {
        ...
        var rightFilterFilePath = fs.path.join(this._settingsPath, env.rightFilterFilename);
        ...
        var calLevelFilePath = fs.path.join(this._settingsPath, env.calLevelsFilename);
    ...
    GridPlayer.prototype.generateStimulus = function (xval, yval, ear) {
        ...
        if (this._hasMasker) {
            ...
            var full_bw_dB = 10 * Math.log10(edge4 - edge1);
            ...
            if (this._compensate) {
                ...
                if (ear === "left") {
                    bs_masker_win_norm = util.calfilter(this._leftFilter, (this._leftCalLevel - this._rightCalLevel), this._maskerLevel + bw_corr_dB, bs_masker_win);
                }
                else if (ear === "right") {
                    bs_masker_win_norm = util.calfilter(this._rightFilter, 0, this._maskerLevel + bw_corr_dB, bs_masker_win);
                }
    ...
    GridPlayer.prototype.generateCalibrationStimulus = function (ear) {
        var _this = this;
        var refToneFilePath = fs.path.join(this._settingsPath, env.refToneFilename);
        var refToneFile = fs.File.fromPath(refToneFilePath);
        var refToneData = refToneFile.readSync(function (err) { _this.log(err); });
        var tmpArray = new ArrayBuffer(refToneData.length);
        refToneData.getBytes(tmpArray);
        var refSignal = new Float32Array(tmpArray);
        console.log("Calibration reference signal abs max value: " + util.max(util.abs(refSignal)));
        var sweepFilePath = fs.path.join(this._settingsPath, env.sweepFilename);
        var sweepFile = fs.File.fromPath(sweepFilePath);
        var sweepData = sweepFile.readSync(function (err) { _this.log(err); });
        tmpArray = new ArrayBuffer(sweepData.length);
        sweepData.getBytes(tmpArray);
        var sweepSignal = new Float32Array(tmpArray);
        var combined_signal = new Float32Array(refSignal.length + Math.round(2 * this._fs) + sweepSignal.length);
        combined_signal.set(refSignal);
        combined_signal.set(sweepSignal, refSignal.length + Math.round(1.8 * this._fs));
        var cal_output = new Float32Array(combined_signal.length);
        if (this._compensate) {
            var cal_norm = void 0;
            if (ear == "left") {
                cal_norm = util.calfilter(this._leftFilter, 6 + (this._leftCalLevel - this._rightCalLevel), 0, combined_signal);
            }
            else if (ear == "right") {
                cal_norm = util.calfilter(this._rightFilter, 6, 0, combined_signal);
            }
